...
>
  <template #notice>
    ...
    <submitToGate1Dialog v-model="submitGate1Dialog" title="Submit to Gate 1 for Review" :status="selectedGatePass.workflow_status" :gatePassNumber="selectedGatePass.pass_number" confirm-text="Yes, Submit" cancel-text="No, Cancel" @confirm="submitToGate1FinalReview">
        <template #notice>
            Are you sure you want to submit this gate pass to Gate 1 security for further review? This action will notify the Gate 1 security team to conduct an additional review before the pass can be approved for entry.
        </template>
    </submitToGate1Dialog>
...
import submitToGate1Dialog from "../SharedComponents/submitToGate1Dialog.vue";
  ...
  setTimeout(() => window.location.reload(), 1000);
   ...
   axios.get(`/gate-passes/${item.id}`)
    ...
    axios.get(url, { responseType: 'blob' })
        ...
        async submitToGate2Review() {

            try {
                const response = await axios.post(`/gate-passes/${this.selectedGatePass.id}/gate2/review`);
